defmodule Trifle.Stats.Transponder.Sum do
  @moduledoc """
  Sum transponder - calculates the sum of scalar values from multiple paths.

  This transponder takes an array of paths, extracts scalar values from each path,
  and computes their total sum. Only numeric values are included in the calculation.

  ## Usage

      # Sum scalar values from multiple paths
      Trifle.Stats.Transponder.Sum.transform(series, ["requests.items", "requests.books"], "requests.total")
  ...
  Returns modified series with sum values added:
      %{
        at: [timestamp1, timestamp2, ...],
        values: [
          %{"requests" => %{"items" => 10, "books" => 5, "shoes" => 15, "total" => 15}},
          %{"requests" => %{"items" => 20, "books" => 8, "shoes" => 12, "total" => 28}},
          ...
        ]
      }
  """
      ...
      transformed_values =
        ...
        |> Enum.map(fn value_map ->
          ...
          if can_create_response_path?(value_map, response_keys) do
            ...
            case calculate_sum(scalar_values) do
              nil -> value_map
              sum_result -> put_path_value(value_map, response_keys, sum_result)
            end
          else
