defmodule Trifle.Stats.Transponder.Multiply do
  @moduledoc """
  Multiply transponder - multiplies two numeric values together.

  This transponder takes two numeric fields and computes their product (left * right).
  Handles missing data gracefully by skipping calculations when either value is missing.

  ## Usage

      # Multiply two fields together
      Trifle.Stats.Transponder.Multiply.transform(series, "requests.value_a", "requests.value_b", "requests.product")
  ...
  Returns modified series with product values added:
      %{
        at: [timestamp1, timestamp2, ...],
        values: [
          %{"requests" => %{"value_a" => 10, "value_b" => 5, "product" => 50}},
          %{"requests" => %{"value_a" => 15, "value_b" => 8, "product" => 120}},
          ...
        ]
      }
  """
  ...
  def transform(series, left, right, response_path, _slices \\ 1) do
    if Enum.empty?(series[:at]) do
      ...
      # Transform values by calculating multiplication
      transformed_values =
        series[:values]
        |> Enum.map(fn value_map ->
          ...
          if can_create_response_path?(value_map, response_keys) do
            ...
            multiply_result = calculate_multiply(left_value, right_value)
            put_path_value(value_map, response_keys, multiply_result)
          else
  ...
  defp resolve_arg(_value_map, _), do: nil
  ...
  # Calculate multiplication with precision handling
  defp calculate_multiply(left, right) when is_number(left) and is_number(right) do
    result = Precision.mult(left, right)
    # Convert to appropriate type based on precision mode
    if Precision.enabled?(), do: result, else: Precision.to_float(result)
  end

  # Handle Decimal values
  defp calculate_multiply(%Decimal{} = left, right) when is_number(right) do
    result = Precision.mult(left, right)
    if Precision.enabled?(), do: result, else: Precision.to_float(result)
  end

  defp calculate_multiply(left, %Decimal{} = right) when is_number(left) do
    result = Precision.mult(left, right)
    if Precision.enabled?(), do: result, else: Precision.to_float(result)
  end

  defp calculate_multiply(%Decimal{} = left, %Decimal{} = right) do
    result = Precision.mult(left, right)
    if Precision.enabled?(), do: result, else: Precision.to_float(result)
  end

  defp calculate_multiply(_left, _right) do
