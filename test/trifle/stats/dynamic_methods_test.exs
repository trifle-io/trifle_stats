defmodule Trifle.Stats.DynamicMethodsTest do
  ...
  defmodule TestNormalizeTransponder do
    ...
    def transform(series, source_path, target_path, _slices \\ 1, _options \\ []) do
      ...
      extracted_values =
        Enum.map(values, &get_in(&1, String.split(source_path, ".")))
      ...
      transformed_values =
        Enum.map(values, fn value ->
          ...
          normalized_value =
            if source_value && granularity > 0 do
  ...
  describe "compile-time method generation" do
    ...
    test "generates transponder methods" do
      ...
      # min value normalized to 0
      assert first_normalized == 0.0
      # max value normalized to 1
      assert second_normalized == 1.0
    end
  ...
  describe "runtime registration" do
    test "components are registered in the registry" do
      ...
      {:ok, TestWeightedAvgAggregator} =
        Trifle.Stats.Series.Registry.get_aggregator(:weighted_avg)
  ...
  describe "integration with fluent interface" do
    ...
    test "can chain between generated and fluent methods" do
      ...
      result =
        ...
        |> TestAnalyticsModule.transpond_normalize("avg", "norm_avg")
        |> aggregate_max("norm_avg")
  ...
  describe "advanced usage patterns" do
    test "can build complex analysis pipelines" do
      ...
      pipeline_result =
        series
        |> transpond_expression(["sum", "count"], "a / b", "avg")
        |> TestAnalyticsModule.transpond_normalize("avg", "norm")
        |> debug_inspect("After normalization")
        |> aggregate_multiple(
          max: ["norm"],
          min: ["norm"],
          mean: ["norm"]
  ...
  describe "error handling" do
    ...
    test "handles missing paths gracefully in generated methods" do
      ...
      # weighted avg should return 0 for missing data
      assert result == 0
    end
  end
end
