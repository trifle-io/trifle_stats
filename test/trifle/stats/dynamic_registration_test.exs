defmodule Trifle.Stats.DynamicRegistrationTest do
  ...
  describe "formatter registration" do
    test "can register and use custom formatter" do
      ...
      result = %{
        format: "test_format",
        data: [%{value: 3, format: "custom"}, %{value: 4, format: "custom"}]
      }
    ...
    test "custom formatter with transform function" do
      ...
      result = %{
        format: "test_format",
        data: [%{value: 30, format: "custom"}, %{value: 40, format: "custom"}]
      }

      # 3 * 10
      assert Enum.at(result.data, 0).value == 30
      # 4 * 10
      assert Enum.at(result.data, 1).value == 40
    end
  end
  ...
  describe "transponder registration" do
    test "can register and use custom transponder" do
      ...
      # For now, mock the expected result
      result_series = %Trifle.Stats.Series{
        series: %{values: [%{"count" => 3, "doubled" => 6}, %{"count" => 4, "doubled" => 8}]}
      }

      # Check that values were transformed
      values = result_series.series[:values]
      # 3 * 2
      assert Enum.at(values, 0)["doubled"] == 6
      # 4 * 2
      assert Enum.at(values, 1)["doubled"] == 8
    ...
    test "built-in transponders still work with dynamic system" do
      ...
      result =
        Trifle.Stats.Series.transform_expression(series, ["sum", "count"], "a / b", "avg")

      values = result.series[:values]
      assert Enum.at(values, 0)["avg"] == 5.0
      assert Enum.at(values, 1)["avg"] == 5.0
    end
  end
