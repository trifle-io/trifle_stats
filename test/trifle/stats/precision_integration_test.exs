defmodule Trifle.Stats.PrecisionIntegrationTest do
  ...
  describe "precision disabled (default behavior)" do
    ...
    test "transponders return float results" do
      {:ok, result} =
        Transponder.Expression.transform(@sample_data, ["sum", "count"], "a / b", "average")
      ...
      {:ok, ratio_result} =
        Transponder.Expression.transform(
          @sample_data,
          ["sum", "count"],
          "(a / b) * 100",
          "percentage"
  ...
  describe "precision enabled mode" do
    ...
    test "sqrt and arithmetic maintain precision" do
      data_with_stats = %{
        at: [~U[2025-08-18 10:00:00Z]],
        values: [
          %{"sum" => 601.765431, "count" => 3, "square" => 141_165.432}
        ]
      }
