defmodule Trifle.Stats.FluentInterfaceTest do
  ...
  describe "formatter methods (terminal operations)" do
    ...
    test "formatter methods support transform functions", %{series: series} do
      ...
      # 10 * 10
      assert Enum.at(entries, 0).value == 100.0
      # 20 * 10
      assert Enum.at(entries, 1).value == 200.0
      # 30 * 10
      assert Enum.at(entries, 2).value == 300.0
    end
  ...
  describe "transformation methods (intermediate operations)" do
    test "transform_expression returns new Series", %{transponder_series: series} do
      result =
        series
      ...
      assert Enum.at(values, 0)["avg"] == 10.0
      assert Enum.at(values, 1)["avg"] == 10.0
    end

    test "can express ratio calculations", %{transponder_series: series} do
      result =
        series
        |> Trifle.Stats.Series.transform_expression(["sum", "count"], "(a / b) * 100", "ratio")

      assert %Trifle.Stats.Series{} = result

      values = result.series[:values]
      assert Enum.at(values, 0)["ratio"] == 1000.0
      assert Enum.at(values, 1)["ratio"] == 1000.0
    end
    ...
    test "can chain transformation to aggregator", %{transponder_series: series} do
      result =
        series
        |> Trifle.Stats.Series.transform_expression(["sum", "count"], "a / b", "avg")
        |> Trifle.Stats.Series.aggregate_max("avg")
