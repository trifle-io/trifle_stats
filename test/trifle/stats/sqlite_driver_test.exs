defmodule Trifle.Stats.SqliteDriverTest do
  ...
  describe "SQLite driver" do
    test "basic operations work" do
      ...
      table_name = "test_sqlite_#{:rand.uniform(1_000_000)}"
      ...
      empty_results =
        Trifle.Stats.Driver.Sqlite.get(to_keys([["non_existent", "hour", 1_692_266_400]]), driver)
    ...
    test "ping/scan operations work" do
      {:ok, conn} = Exqlite.start_link(database: ":memory:")
      table_name = "test_sqlite_ping_#{:rand.uniform(1_000_000)}"
    ...
    test "multiple keys work independently" do
      {:ok, conn} = Exqlite.start_link(database: ":memory:")
      table_name = "test_sqlite_multi_#{:rand.uniform(1_000_000)}"
    ...
    test "JSON operations work correctly" do
      {:ok, conn} = Exqlite.start_link(database: ":memory:")
      table_name = "test_sqlite_json_#{:rand.uniform(1_000_000)}"
    ...
    test "separated identifier mode operations" do
      {:ok, conn} = Exqlite.start_link(database: ":memory:")
      table_name = "test_sqlite_separated_#{:rand.uniform(1_000_000)}"
      ...
      keys =
        to_keys([["user_events", "hour", 1_692_266_400], ["user_events", "day", 1_692_230_400]])
    ...
    test "separated mode requires RFC3339 timestamps" do
      {:ok, conn} = Exqlite.start_link(database: ":memory:")
      table_name = "test_sqlite_timestamp_formats_#{:rand.uniform(1_000_000)}"
      ...
      Exqlite.query!(
        ...
        VALUES ('legacy_metric', '1d', '2026-02-25 00:00:00', json('{"count": 9}'))
        """,
        []
      )
      ...
      Exqlite.query!(
        ...
        """
        INSERT INTO #{table_name} (key, granularity, at, data)
        VALUES ('iso_metric', '1d', '2026-02-26T00:00:00Z', json('{"count": 7}'))
        """,
        []
      )
    ...
    test "separated mode stores RFC3339 timestamps" do
      {:ok, conn} = Exqlite.start_link(database: ":memory:")
      table_name = "test_sqlite_rfc3339_#{:rand.uniform(1_000_000)}"
      driver = Trifle.Stats.Driver.Sqlite.new(conn, table_name, nil, nil)

      assert :ok = Trifle.Stats.Driver.Sqlite.setup!(conn, table_name, nil)

      at = ~U[2026-02-25 00:00:00Z]
      key = Trifle.Stats.Nocturnal.Key.new(key: "rfc_metric", granularity: "1d", at: at)

      Trifle.Stats.Driver.Sqlite.set([key], %{count: 1}, driver)

      {:ok, result} =
        Exqlite.query(
          conn,
          "SELECT at FROM #{table_name} WHERE key = 'rfc_metric' AND granularity = '1d' LIMIT 1;",
          []
        )

      assert [[stored_at]] = result.rows
      assert stored_at == DateTime.to_iso8601(at)
